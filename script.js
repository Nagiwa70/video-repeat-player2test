document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- プレイヤーインターフェース (HTML5とYouTubeを抽象化) ---
    ...
    // --- 初期化 ---
    if (isMobile) playerWrapper.classList.add('is-mobile');
    ...
    function onYoutubePlayerError(event) {
        if (event.data === 101 || event.data === 150) {
            showToast('この動画は埋め込みが許可されていません。');
        } else {
            showToast(`YouTubeプレーヤーエラーが発生しました (コード: ${event.data})`);
        }
        if(youtubePlayer) {
            youtubePlayer.destroy();
            youtubePlayer = null;
    ...
    function setStartTime(time) {
        const duration = playerInterface.getDuration();
        if (!duration) return;
        const newStartTime = (time !== undefined) ? time : playerInterface.getCurrentTime();
    ...
    function setEndTime(time) {
        const duration = playerInterface.getDuration();
        if (!duration) return;
        const newEndTime = (time !== undefined) ? time : playerInterface.getCurrentTime();
        if (repeatStart === null || newEndTime <= repeatStart) {
            showToast(repeatStart === null ? '先に開始位置を設定してください。' : '終了位置は開始位置より後に設定してください。');
            return;
        }
    ...
    function resetRepeat(silent = false) {
        repeatStart = null;
        repeatEnd = null;
        updateUI();
        if (!silent) showToast('リピート区間をリセットしました。');
    }

    function goToStart() {
        if (repeatStart !== null) playerInterface.seekTo(repeatStart);
    }
    ...
    function skipTime(seconds) {
        ...
        const duration = playerInterface.getDuration();
        if (!duration) return;
        playerInterface.seekTo(Math.max(0, Math.min(duration, currentTime + seconds)));
    }
    ...
    function setPlaybackRate(rateStr) {
        const rate = parseFloat(rateStr);
        if (isNaN(rate)) return;
        const validatedRate = Math.max(0.25, Math.min(rate, 4));
        playerInterface.setPlaybackRate(validatedRate);
        speedRange.value = validatedRate;
        speedValue.value = validatedRate.toFixed(2);
    }
    ...
    function updateMarkers() {
        ...
        if (repeatStart !== null && repeatEnd !== null) {
            ...
            repeatRangeBar.style.width = `${endPercent - startPercent}%`;
    ...
    function updateProgressUI() {
        const duration = playerInterface.getDuration();
        if (!duration) return;
        const percent = (playerInterface.getCurrentTime() / duration) * 100;
    ...
    function updateTimeDisplay() {
        const current = formatTime(playerInterface.getCurrentTime());
        const duration = formatTime(playerInterface.getDuration());
    ...
    function handleTimeUpdate() {
        ...
        if (repeatStart !== null && repeatEnd !== null) {
            const currentTime = playerInterface.getCurrentTime();
            if (currentTime >= repeatEnd || currentTime < repeatStart) {
                playerInterface.seekTo(repeatStart);
            }
        }
    }

    function updateCentralPlayPauseIcon() {
        const iconElement = isMobile ? mobilePlayPauseIcon : pcFeedbackOverlay.querySelector('#playPauseIcon');
        if (iconElement) {
            iconElement.className = playerInterface.isPaused() ? 'play' : 'pause';
        }
    }

    function flashPcPlayPauseIcon() {
        if (isMobile) return;
        pcFeedbackOverlay.innerHTML = `<div id="playPauseIcon" class="${playerInterface.isPaused() ? 'pause' : 'play'}"></div>`;
        pcFeedbackOverlay.classList.add('feedback-flash');
        setTimeout(() => pcFeedbackOverlay.classList.remove('feedback-flash'), 500);
    }
    
    function hideMobileControls() {
        playerWrapper.classList.remove('mobile-controls-visible');
        clearTimeout(controlsTimer);
    }

    function showMobileControls() {
        playerWrapper.classList.add('mobile-controls-visible');
        updateCentralPlayPauseIcon();
        clearTimeout(controlsTimer);
    ...
    function resetInactiveTimer() {
        ...
        if (document.fullscreenElement && !isMobile) {
            inactiveTimer = setTimeout(() => {
                playerWrapper.classList.add('is-inactive');
            }, 3000);
        }
    }
    ...
    const handleDragMove = (e) => {
        if (!isDragging || !playerInterface.getDuration()) return;
        const clientX = e.touches ? e.touches[0].clientX : e.clientX;
        const rect = progressBarContainer.getBoundingClientRect();
        const ratio = Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));
        playerInterface.seekTo(ratio * playerInterface.getDuration());
        updateProgressUI();
    };
    ...
    const handleDragStart = (e) => {
        if (!playerInterface.getDuration() || e.shiftKey || e.altKey) return;
        e.preventDefault();
        isDragging = true;
    ...
    urlInputContainer.addEventListener('submit', (e) => {
        e.preventDefault();
        const url = youtubeUrlInput.value;
        const videoId = url.match(/(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|\S*?[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})/);
        if (videoId) {
            loadYoutubeVideo(videoId[1]);
        } else {
            showToast('無効なYouTube URLです。');
        }
    });
    ...
    progressBarContainer.addEventListener('click', (e) => {
        if (!playerInterface.getDuration() || isDragging) return;
        const rect = progressBarContainer.getBoundingClientRect();
        const ratio = (e.clientX - rect.left) / rect.width;
        const clickedTime = playerInterface.getDuration() * ratio;

        if (e.shiftKey && e.altKey) { setEndTime(clickedTime); }
        else if (e.shiftKey) { setStartTime(clickedTime); }
        else { playerInterface.seekTo(clickedTime); }
    });
    progressBarContainer.addEventListener('mousemove', (e) => {
        if (!playerInterface.getDuration()) return;
        const rect = progressBarContainer.getBoundingClientRect();
        const hoverRatio = Math.max(0, Math.min(1, (e.clientX - rect.left) / rect.width));
        progressTimeTooltip.textContent = formatTime(hoverRatio * playerInterface.getDuration());
        progressTimeTooltip.style.left = `${hoverRatio * 100}%`;
    });

    playerWrapper.addEventListener('click', (e) => {
        const clickedOnUI = videoControls.contains(e.target);
        if (isMobile) {
            if (playerWrapper.classList.contains('mobile-controls-visible')) {
                if (e.target === mobilePlayPauseIcon) {
                    togglePlayPause();
                    hideMobileControls();
                } else if (!clickedOnUI) {
                    hideMobileControls();
